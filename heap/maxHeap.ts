export class MaxHeap {
  ...
  private heapifyDown() {
    ...
    while (this.hasLeftChild(index)) {
      let largerElemetIndex = this.getLeftChildIndex(index);
      if (
        this.hasRightChild(index) &&
        this.rightChild(index) > this.leftChild(index)
      ) {
        largerElemetIndex = this.getRightChildIndex(index);
      }

      if (this.array[index] < this.array[largerElemetIndex]) {
        this.swap(index, largerElemetIndex);
      } else {
        break;
      }
      index = largerElemetIndex;
    }
  }
}
